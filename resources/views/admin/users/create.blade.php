<x-app-layout>
    <style>
        ...
        .form-input-v2, .form-select-v2 {
            ...
            font-size: 1rem;
            position: relative;
            z-index: 1; /* Nivel sobre la label */
        }
        ...
        /* Lógica para la label flotante */
        .form-label-v2 {
            position: absolute;
            left: 1rem;
            top: 1rem;
            font-size: 1rem;
            color: rgb(var(--color-mid-text));
            pointer-events: none;
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
            z-index: 2; /* Nivel sobre el input */
            background: linear-gradient(to bottom, rgb(var(--color-dark)), rgb(var(--color-darker)));
            padding: 0 0.5rem;
            border-radius: 0.5rem;
        }

        /* Movimiento de la label al hacer focus o si el input tiene valor */
        ...
        /* === ESTILOS ESPECÍFICOS PARA EL SELECT MEJORADO === */
        .form-select-v2 {
            appearance: none;
        ...
        /* Solución a la legibilidad: color de placeholder para el estado inicial */
        .form-select-v2:required:invalid {
            color: rgb(var(--color-mid-text));
        }
        /* Color normal una vez se ha seleccionado una opción válida */
        .form-select-v2:valid {
            color: rgb(var(--color-light-text));
        }
        /* Nota: el estilo del menú desplegable <option> es controlado por el OS/Navegador */
        ...
        .form-error-message {
            ...
            color: rgb(248, 113, 113); /* Un rojo más claro para legibilidad */
            font-size: 0.875rem;
            border-radius: 0.5rem;
            margin-top: 0.5rem;
        }
        ...
        .btn-primary-yellow:not(:disabled):hover {
            transform: translateY(-2px) scale(1.02);
            box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.2);
        }
        ...
        .btn:disabled {
            opacity: 0.6;
            cursor: not-allowed;
            background: linear-gradient(135deg, rgb(var(--color-primary) / 0.6), rgb(var(--color-primary-dark) / 0.6));
        }
        ...
        .btn-secondary-ghost {
            ...
            color: rgb(var(--color-light-text));
            border: 2px solid rgba(var(--color-light-text), 0.2);
            padding: 0.75rem 1.75rem;
            border-radius: 0.75rem;
        }
        
        .btn-secondary-ghost:hover {
            background: rgba(var(--color-light-text), 0.05);
        ...
        .divider-gradient {
            height: 2px;
            background: linear-gradient(90deg, transparent, rgba(var(--color-secondary), 0.5), rgba(var(--color-primary), 0.7), rgba(var(--color-secondary), 0.5), transparent);
            margin: 2.5rem 0;
            position: relative;
            overflow: hidden;
        }
        ...
        <div class="flex items-center gap-4 animate-in">
            <div class="icon-pulse-glow">
                <svg class="w-12 h-12 text-yellow-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 0110.5 22.5c-2.948 0-5.757-1.225-7.754-3.265z" />
                </svg>
            </div>
    ...
    <div class="py-12 relative overflow-hidden"> <!-- `overflow-hidden` para contener las partículas -->
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="gradient-border-card">
                <div class="card-content">
                    <form method="POST" action="{{ route('admin.users.store') }}" id="register-form">
                        @csrf

                        <!-- Sección 1: Datos Personales y Rol -->
                        <div class="animate-in">
                            <div class="flex items-center gap-3 mb-8">
                                <svg class="w-8 h-8 text-purple-400 icon-float" style="animation-delay: 0.2s" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>
                                <h3 class="text-2xl font-bold text-gray-200">Identificación y Rol</h3>
                            </div>
                            <div class="space-y-8">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div class="input-group">
                                        <input id="name" type="text" name="name" class="form-input-v2" placeholder=" " required />
                                        <label for="name" class="form-label-v2">Nombre Completo</label>
                                        {{-- El mensaje de error ahora usará la clase personalizada --}}
                                        <x-input-error :messages="$errors->get('name')" class="form-error-message" />
                                    </div>
                                    <div class="input-group">
                                        <input id="email" type="email" name="email" class="form-input-v2" placeholder=" " required />
                                ...
                                {{-- SECCIÓN DEL SELECT MEJORADA --}}
                                <div class="select-group">
                                    <select id="rol" name="rol" class="form-select-v2" required>
                                        {{-- El value="" es clave para que :required:invalid funcione --}}
                                    ...
                                    {{-- La label ahora es hermana del select, para que la CSS `~` funcione --}}
                        ...
                        <div class="animate-in">
                            <div class="flex items-center gap-3 mb-8">
                                <svg class="w-8 h-8 text-purple-400 icon-float" style="animation-delay: 0.4s" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" /></svg>
                                <h3 class="text-2xl font-bold text-gray-200">Credenciales de Seguridad</h3>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div class="input-group">
                                    <input id="password" type="password" name="password" class="form-input-v2" placeholder=" " required />
                                    <label for="password" class="form-label-v2">Contraseña</label>
                                    <x-input-error :messages="$errors->get('password')" class="form-error-message" />
                                </div>
                                <div class="input-group">
                                    <input id="password_confirmation" type="password" name="password_confirmation" class="form-input-v2" placeholder=" " required />
                        ...
                        <div class="mt-12 pt-8 border-t-2 border-dashed border-gray-700/50 flex items-center justify-end gap-4 animate-in">
                            <a href="{{ route('admin.users.index') }}" class="btn btn-secondary-ghost">
                                <svg class="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                                Cancelar
                            </a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            // Animación de entrada para los elementos marcados
            ...
            // --- Script de partículas mejorado ---
            const particlesContainer = document.getElementById('particles-js');
            if (particlesContainer) {
                const particleCount = 25; // Aumentamos un poco
                const colors = [
                    'var(--color-primary)', 
                    'var(--color-secondary)',
                    'var(--color-accent)'
            ...
            // --- Script para feedback del formulario ---
            ...
            if (form) {
                form.addEventListener('submit', (e) => {
                    ...
                    buttonSpinner.insertAdjacentText('afterend', ' Registrando...');
                });
            }
        });
    </script>
